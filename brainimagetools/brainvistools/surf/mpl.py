...
def normal_vectors(vertices, faces):
    """Calculate normal vectors for faces"""
...
#     fig = plt.figure(
#         figsize=(
#             base_size * len(rotate) + colorbar * (base_size - 2),
...
#                 C, pvals, F, mask, mask_colour=mask_colour, border_colour=border_colour
#             )
...
#                 A_base = (
#                     np.c_[vertices + vertex_normal_orig * 0.01, np.ones(len(vertices))]
#                     @ MVP.T
#                 )
...
#         cbar.ax.set_title(cmap_label, fontsize=12, pad=30)

#     plt.subplots_adjust(left=0.2, right=0.6, top=0.8, bottom=0.2, wspace=-0.2, hspace=0)
#     if return_ax:
#         return fig, ax, MVP
#     plt.close()
#     return fig
...
def surf2ax(
    ...
    **kwargs,
    ...
    if threshold is not None:
        overlay = np.where(overlay > threshold, overlay, 0)
    ...
    collection.set_alpha(transparency)
    collection.set_path_effects([Stroke(linewidth=0, foreground="none"), Normal()])

    ax.add_collection(collection)


def plot_fsLR(
    data, hemi="left", surface="inflated", atlas=None, view="lateral", ax=None, **kwargs
):
    vertices, faces = hcp.mesh[f"{surface.lower()}_{hemi}"]

    if hemi.lower() in ["left", "lh", "l"]:
        hemi = "left"
        if len(data) != len(vertices):
            overlay = hcp.left_cortex_data(data)
        else:
            overlay = data

        atlas = hcp.left_cortex_data(hcp.yeo7.map_all)

    elif hemi.lower() in ["right", "rh", "r"]:
        hemi = "right"
        if len(data) != len(vertices):
            overlay = hcp.right_cortex_data(data)
        else:
            overlay = data
        atlas = hcp.right_cortex_data(hcp.yeo7.map_all)

    if isinstance(atlas, str):
        label_atlas = hcp.__dict__[atlas].rgba
    else:
        label_atlas = None

    if hemi in ["both", "LR"]:
        hemi = "both"
        vertices, faces = hcp.mesh[f"{surface.lower()}"]
        if len(data) != len(vertices):
            overlay = hcp.cortex_data(data)
        else:
            overlay = data
        atlas = hcp.cortex_data(hcp.yeo7.map_all)
        label_atlas = hcp.yeo7.rgba

    if hemi == "left" and view == "lateral":
        rotation = 90
    if hemi == "left" and view == "medial":
        rotation = 270

    if isinstance(ax, type(None)):
        fig, ax = plt.subplots(ncols=1, layout="tight", figsize=(16, 8))
        return_fig = True
    else:
        return_fig = False

    surf2ax(vertices, faces, overlay=overlay, ax=ax, view=rotation, **kwargs)
