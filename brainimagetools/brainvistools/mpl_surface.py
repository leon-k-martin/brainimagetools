"""
Code from https://github.com/kwagstyl/matplotlib_surface_plotting
"""
...
def add_parcellation_colours(
    colours, parcel, triangles, labels=None, mask=None, filled=False, mask_colour=None
):
    """delineate regions"""
...
def plot_surf(
    ...
    for k, overlay in enumerate(overlays):
        ...
        collection = PolyCollection(
            [], closed=True, linewidth=0, antialiased=False, facecolor=C, cmap=cmap
        )
        for i, view in enumerate(rotate):
            ...
            if isinstance(ext_ax, type(None)):
                ax = fig.add_subplot(
                    len(overlays),
                    len(rotate) + 1,
                    2 * k + i + 1,
            ...
            collection = PolyCollection(
                T, closed=True, linewidth=0, antialiased=False, facecolor=s_C, cmap=cmap
            )
